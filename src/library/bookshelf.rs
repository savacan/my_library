...
impl Bookshelf {
    ...
    pub fn search_books_exact(&self, title_query: &str) -> Vec<&Book> {
        self.books.iter().filter(|book| book.title == title_query).collect()
    }
    // タイトル名の部分一致で本を検索するメソッド
    pub fn search_books_partial(&self, title_query: &str) -> Vec<&Book> {
        self.books.iter().filter(|book| book.title.contains(title_query)).collect()
    }
}
...
mod tests {
    ...
    fn test_bookshelf() {
        ...
        let found_books = shelf.search_books("入門");
        println!("{:?}", found_books);
    }
}